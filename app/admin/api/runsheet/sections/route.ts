...
export async function POST(request: NextRequest) {
  ...
  const parsed = parseSectionFields(body, { requireCore: true });
  if ('error' in parsed) {
  ...
  if (error) {
    logErr('insert section', error);
    return NextResponse.json({ message: 'Failed to create section', details: error.message }, { status: 500 });
  }
